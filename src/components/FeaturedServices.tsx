...
export function FeaturedServices({ services }: Props) {
  ...
  return (
    <section ref={sectionRef} className="bg-[#f8f9fa] px-0 py-24">
      <div className="mx-auto max-w-6xl px-6 md:px-8 lg:px-12">
        <div className="text-center mb-14">
          <h2 className="text-5xl font-bold text-[#2c3e50] md:text-5xl">Featured Services</h2>
        </div>
        <div className="grid grid-cols-2 gap-6 sm:grid-cols-3 lg:grid-cols-3">
          {services.map((service, index) => {
            ...
            return (
              <div
                key={service.title}
                className={`group flex h-full flex-col items-center gap-3 rounded-2xl bg-white p-6 text-center shadow-[0_4px_12px_rgba(0,0,0,0.08)] transition-all duration-500 ${
                  isActive
              ...
              >
                ...
                {description && (
                  <p className="text-sm leading-relaxed text-[#4b5563] max-w-[220px]">
                    {description}
                  </p>
                )}
              </div>
